...
To obtain the coefficients of each exponential decay, we can use the normalised intensity values output from the NIL (\autoref{multiexp}) or take the central value of the two peaks for the rate constant, and use the NIL algorithm to extract the coefficients.
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \label{dep_plot_2}
\end{figure}
\noindent{By plotting the length in a log scale, the collision probability has a linear dependence on the base length. These estimates are comparable to studies done by Plaxco \textit{et al}. (\autoref{plaxco_plot} \cite{arroyo2018subsecond}), who found apparent electron transfer rate exhibits a power-law dependence on chain length, N (number of monomers), 
